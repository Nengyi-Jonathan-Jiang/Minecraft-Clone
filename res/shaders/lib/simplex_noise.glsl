...
#define permute_f(x) (floor(permute(x)))
#define taylorInvSqrt(r) (1.79284291400159 - 0.85373472095314 * (r))

vec4 _simplex_noise_grad4(float j, vec4 ip) {
...
float simplex_noise(vec4 v) {
    ...
    vec4 p0 = _simplex_noise_grad4(j0, ip);
    vec4 p1 = _simplex_noise_grad4(j1.x, ip);
    vec4 p2 = _simplex_noise_grad4(j1.y, ip);
    vec4 p3 = _simplex_noise_grad4(j1.z, ip);
    vec4 p4 = _simplex_noise_grad4(j1.w, ip);
