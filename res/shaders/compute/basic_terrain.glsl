...
void main() {
    uint result = 0;
    ivec3 worldPos = ivec3(gl_WorkGroupID.xyz);
...
float _dot(uvec3 g, vec2 v) {
    return dot(vec3(g).xy, v);
}

float _dot(uvec3 g, vec3 v) {
    return dot(vec3(g), v);
}
